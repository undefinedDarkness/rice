...
local create_button = function(text, command)
    local icon =
        ...
        C.hover_effect(
            ...
            function(w, m)
                if m then
                    w.font = "Sarasa UI HC Bold 10"
                else
                    w.font = "Sarasa UI HC 10"
                end
            end
        )
    )
...
music_image_widget:buttons(
    awful.button(
        ...
        mouse.MIDDLE,
        function()
            music_image_widget:get_children_by_id("image")[1]:set_image(
                gears.surface.load_uncached(user_home .. "/.config/awesome/theme/record.jpg")
            )
        end
    )
)

music_image_widget:connect_signal(
    "mouse::enter",
    function()
